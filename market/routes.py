    ...
    if request.method == 'POST':
        ...
        if s_item_obj:
            if current_user.can_sell(s_item_obj):
                ...
                flash(f'You sucessfully sold {s_item_obj.name} for {s_item_obj.price}$', category='success')
            else:
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        login_user(new_user)
        flash(f'Account createt succesfully! You are now logged in as {new_user.username}. Happy shopping', category='success')
        return redirect(url_for('market_page'))
